...
function _getAppleIconQualifier(
  iconSize: number,
...
async function _saveDefaultIconToPathAsync(
  ...
  if (context instanceof StandaloneContextUser) {
    const { data } = context;
    if (data.exp.icon) {
      await saveImageToPathAsync(data.projectPath, data.exp.icon, filePath);
    } else {
    ...
    if (data.manifest.ios && data.manifest.ios.iconUrl) {
      await saveUrlToPathAsync(data.manifest.ios.iconUrl, filePath);
    } else if (data.manifest.iconUrl) {
      await saveUrlToPathAsync(data.manifest.iconUrl, filePath);
    } else {
      throw new Error(
        'Cannot save icon because manifest has no iconUrl or ios.iconUrl key.',
      );
    }
  }
}
...
async function createAndWriteIconsToPathAsync(
  ...
  try {
    await _saveDefaultIconToPathAsync(
      context,
    ...
    iconSizes.map(async (iconSize) => {
      ...
      await Promise.all(
        iconResolutions.map(async (iconResolution) => {
          const iconQualifier = _getAppleIconQualifier(
            iconSize,
            iconResolution,
          );
