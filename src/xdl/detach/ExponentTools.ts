...
function parseSdkMajorVersion(expSdkVersion: string) {
  ...
  try {
    const versionComponents = expSdkVersion
      .split('.')
...
async function saveUrlToPathAsync(
  url: string,
  filePath: string,
  timeout = 20000,
) {
...
async function saveImageToPathAsync(
  projectRoot: string,
  pathOrURL: string,
  outPath: string,
) {
...
async function getManifestAsync(url: string, headers: any, options: any = {}) {
  const buildPhaseLogger =
    options.logger ||
    LoggerDetach.withFields({ buildPhase: 'reading manifest' });

  let response;
  try {
    response = await _retryPromise(() =>
      axios.get(url.replace('exp://', 'http://'), { headers }),
    );
  } catch (err) {
...
async function _retryPromise<T>(
  fn: (...args: any[]) => T,
  retries = 5,
): Promise<T> {
  try {
  ...
  if (exp.locales !== undefined) {
    for (const [lang, localePath] of Object.entries(exp.locales)) {
      const s = await fs.readFile(
        path.resolve(projectRoot, localePath as string),
        'utf8',
      );
      try {
...
async function deleteLinesInFileAsync(
  ...
  for (const line of lines) {
    ...
    if (inDeleteRange && line.match(endRegex)) {
      inDeleteRange = false;
    }
  }
