...
import turtleLogger from 'turtle/logger';
...
function _configureInfoPlistForLocalDevelopment(
  config: any,
  exp: ExpoConfig,
): ExpoConfig {
  // add detached scheme
...
function _logDeveloperInfoForLocalDevelopment(infoPlist: any[]): void {
  ...
  if (usageKeysConfigured.length) {
    ...
    logger.info(
      'You may want to revise them to include language appropriate to your project. ' +
        'You can also remove them if your app will never use the corresponding API. ' +
        'See the Apple docs for these keys.',
    );
  }
}
...
async function _preloadManifestAndBundleAsync(
  ...
  await fs.writeFile(
    path.join(supportingDirectory, manifestFilename),
    JSON.stringify(manifest),
  );
  await saveUrlToPathAsync(
    bundleUrl,
    path.join(supportingDirectory, bundleFilename),
  );
}
...
async function _maybeLegacyPreloadKernelManifestAndBundleAsync(
  ...
  const sdkVersionSupported = await IosWorkspace.getNewestSdkVersionSupportedAsync(
    context,
  );
 ...
 */
async function _configureEntitlementsAsync(
  context: AnyStandaloneContext,
): Promise<any> {
  if (context instanceof StandaloneContextUser) {
    ...
    logger.info(
      'Your iOS ExpoKit project will not contain an .entitlements file by default. ' +
        'If you need specific Apple entitlements, enable them manually via Xcode or the Apple Developer website.',
    );
    const keysToFlag = [];
    ...
    const manifest =
      isStandaloneContextDataService(context.data) && context.data.manifest;
    ...
    if (!fs.existsSync(path.join(supportingDirectory, entitlementsFilename))) {
      await IosPlist.createBlankAsync(
        supportingDirectory,
    ...
    const result = IosPlist.modifyAsync(
      ...
      (entitlements) => {
        ...
        entitlements['aps-environment'] =
          context.build.configuration === 'Release'
            ? 'production'
            : 'development';
        ...
        if (manifest.ios?.usesAppleSignIn) {
          ...
          entitlements.hasOwnProperty('com.apple.developer.applesignin')
        ) {
          delete entitlements['com.apple.developer.applesignin'];
        }

        if (manifest.ios?.accessesContactNotes) {
          entitlements['com.apple.developer.contacts.notes'] =
            manifest.ios.accessesContactNotes;
        } else if (
          entitlements.hasOwnProperty('com.apple.developer.contacts.notes')
        ) {
        ...
        if (manifest.ios?.associatedDomains) {
          ...
          entitlements.hasOwnProperty('com.apple.developer.associated-domains')
        ) {
 ...
 */
async function _configureInfoPlistAsync(
  context: AnyStandaloneContext,
): Promise<void> {
  const { supportingDirectory } = IosWorkspace.getPaths(context);
  ...
  const result = await IosPlist.modifyAsync(
    supportingDirectory,
    'Info',
    (infoPlist) => {
      ...
      if (parseSdkMajorVersion(config.sdkVersion) >= 36) {
        infoPlist.FacebookAutoInitEnabled =
          config.facebookAutoInitEnabled || false;
        infoPlist.FacebookAutoLogAppEventsEnabled =
          config.facebookAutoLogAppEventsEnabled || false;
      ...
      infoPlist.GADApplicationIdentifier =
        (privateConfig && privateConfig.googleMobileAdsAppId) ||
        DEFAULT_GAD_APPLICATION_ID;
      ...
      for (const key in infoPlist) {
        if (
          ...
          infoPlist[key] = infoPlist[key].replace(
            'Expo experiences',
            permissionsAppName,
          );
        }
      ...
      infoPlist.UIDeviceFamily =
        config.ios && config.ios.supportsTablet ? [1, 2] : [1];
      ...
      infoPlist.UIRequiresFullScreen =
        config.ios && config.ios.requireFullScreen;
      if (infoPlist.UIRequiresFullScreen == null) {
        // NOTES: This is defaulted to `true` for now to match the behavior prior to SDK 34,
        // but will change to `false` in a future SDK version.
      ...
      if (userInterfaceStyle) {
        ...
        infoPlist.UIUserInterfaceStyle = _mapUserInterfaceStyleForInfoPlist(
          userInterfaceStyle,
        );
      }
      ...
      if (context instanceof StandaloneContextUser) {
        infoPlist = _configureInfoPlistForLocalDevelopment(
          infoPlist,
          context.data.exp,
        );
 ...
 */
async function _configureShellPlistAsync(
  context: AnyStandaloneContext,
): Promise<void> {
  const { supportingDirectory } = IosWorkspace.getPaths(context);
  const config = context.config;
  const buildPhaseLogger = logger.withFields({
  ...
  await IosPlist.modifyAsync(supportingDirectory, 'EXShell', (shellPlist) => {
    ...
    if (
      isStandaloneContextDataService(context.data) &&
      context.data.testEnvironment
    ) {
    ...
    if (config.updates?.hasOwnProperty('fallbackToCacheTimeout')) {
      shellPlist.updatesFallbackToCacheTimeout =
        config.updates.fallbackToCacheTimeout;
    }
    if (
      !manifestUsesSplashApi(config, 'ios') &&
      parseSdkMajorVersion(config.sdkVersion) < 28
    ) {
...
async function _configureGoogleServicesPlistAsync(
  context: AnyStandaloneContext,
): Promise<void> {
  if (context instanceof StandaloneContextUser) {
    return;
...
export async function configureAsync(
  context: AnyStandaloneContext,
): Promise<void> {
  const buildPhaseLogger = logger.withFields({
    buildPhase: 'configuring NSBundle',
  ...
  try {
    ...
    buildPhaseLogger.info(
      `Modifying NSBundle configuration at ${supportingDirectory}...`,
    );
...
function _mapUserInterfaceStyleForInfoPlist(
  ...
  switch (userInterfaceStyle) {
    ...
    default:
      logger.warn(
        `User interface style "${userInterfaceStyle}" is not supported. ` +
          `Supported values: "light", "dark", "automatic".`,
      );
  }
