...
export async function bundleAsync(
  ...
  await pMap(assets, (asset) => downloadAssetAsync(urlResolver, dest, asset), {
    concurrency: 5,
  });
}

async function downloadAssetAsync(
  ...
  logger.info(urlResolver(hash));
  await pRetry(
    () => saveUrlToPathAsync(urlResolver(hash), path.join(dest, asset)),
    { retries: 3 },
  );
}
